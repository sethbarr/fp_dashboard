# Note: this model applies the conversion rate to the starting population sizes to calculate the number of users for each year and iterates on that changing population for 4 years. This model may be more generalizable than the `dashboard.py` model but does not have all of the features in the `dashboard.py` model.
import dash
