        ...
        html.Div([
            ...
            html.Div([
                ...
                html.Div([
                    html.Div([
                        html.Label("Year 1"),
                        dcc.Input(id='dmpim-conv-rate-1', type='number', value=15.8417599) # weird first value for scenario 1
                    ], className='input-group'),
                    html.Div([
                        html.Label("Year 2"),
...
def update_graph(submit_n_clicks, export_n_clicks, neten_start_pop, dmpim_start_pop, dmpsc_start_pop, 
                 ...
                 neten_color, dmpim_color, dmpsc_color, cost_saving_color):
    ...
    for i in range(years):
        if user_pop_sizes[i] is not None:
            ...
            n_dmpsc = int(dmpim_start_pop * (dmpim_Conv[i])) + int(manual_neten_pop_sizes[i]) * (neten_Conv[i])
        elif neten_start_pop == 0:
            n_dmpim = int(dmpim_start_pop * (1-dmpim_Conv[i]))
            # n_neten = int(manual_neten_pop_sizes[i] * (1-neten_Conv[i]))
            n_dmpsc = int(dmpim_start_pop * (dmpim_Conv[i]))
    ...
    neten_visit_costs = [n * neten_visit_cost for n in neten] #if n > 0 else 0 for n in neten]

    dmpim_product_costs = [d * dmpim_product_cost for d in dmpim]
    neten_product_costs = [n * neten_product_cost for n in neten] #if n > 0 else 0 for n in neten]
    dmpsc_product_costs = [d * dmpsc_product_cost for d in dmpsc]
    
    
        
    # Baseline costs. IM and SC costs are the same for the first 4 years, NETEN costs vary slightly based on the manual population sizes
    baseline_dmpim_visit_cost = [dmpim_start_pop * dmpim_visit_cost] * 4
    baseline_dmpsc_visit_cost = [dmpsc_start_pop * dmpsc_visit_cost] * 4
    baseline_neten_visit_cost = [n * neten_visit_cost if n > 0 else 0 for n in manual_neten_pop_sizes]
    baseline_neten_product_cost = [n * neten_product_cost if n > 0 else 0 for n in manual_neten_pop_sizes]
    baseline_dmpim_product_cost = [dmpim_start_pop * dmpim_product_cost] * 4
    baseline_dmpsc_product_cost = [dmpsc_start_pop * dmpsc_product_cost] * 4

    # baselinecost = [sum(x) for x in zip(dmpim_visit_costs, dmpim_product_costs, 
    #                                  dmpsc_visit_costs, dmpsc_product_costs, 
    #                                  neten_visit_costs, neten_product_costs)]
    
    baselinecost = sum(x[0] for x in [dmpim_visit_costs, dmpim_product_costs, neten_visit_costs, neten_product_costs, dmpsc_visit_costs, dmpsc_product_costs])
